...
tibble(
  name = param_sets,
  parameters = map(name, load_scenario),
...
params_historical <- load_scenario("historical")
R0_historical <- calculate_R0(params_historical)

params_modern <- load_scenario("modern-estimates")  
...
for (intervention in interventions) {
  if (intervention == "baseline") {
    params_list <- list(scenario = "modern-estimates")
  } else if (intervention == "rat_control") {
    params_list <- list(scenario = "modern-estimates", K_r = 1500)  # 50% reduction
  } else {
    params_list <- list(scenario = "modern-estimates", K_f = 3.0)   # Reduced flea capacity
  }
...
scenarios_r0 <- data.frame(
  ...
  R0 = c(
    calculate_R0(load_scenario("modern-estimates")),
    calculate_R0(load_scenario("modern-estimates", beta_r = 7.0)),
    calculate_R0(load_scenario("modern-estimates", K_f = 10.0, K_r = 4000))
  )
)
