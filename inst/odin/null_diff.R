...
## `seasonal_beta` is a per-day multiplier applied to BOTH transmission rates.
## Rat and human infection are mediated by the same flea pool, so they carry
## the same response. Anchored at its own maximum (w <= 1) by the R-side
...
lambda_r <- if(T_r > 0) beta_r * w_beta * Q * (1 - exp(-rho * T_r / K_r)) / T_r else 0

# From carcasses to humans: fleas that don't find rats reach humans
lambda_h <- if(K_r > 0) beta_h * w_beta * Q * exp(-rho * T_r / K_r) / K_r else 0
...
I_ini <- parameter(differentiate = TRUE, 10)          # initial infected rats (absolute count)
...
beta_r <- parameter(differentiate = TRUE, 0.77)       # transmission rate from carcasses to rats (per day)
beta_h <- parameter(differentiate = TRUE, 0.0145)     # transmission rate from carcasses to humans (per day)
...
lambda_baseline <- parameter(differentiate = TRUE, 1) # baseline non-plague deaths reported as plague (per day)
